...
repoEntrySchema.virtual('includable').get(function () {
	return [EntryType.VerilogFile, EntryType.IP, EntryType.CFile, EntryType.HFile, EntryType.Folder].includes(this.handler);
});
	...
	if (this.handler === EntryType.RepoRoot) {
		...
		return Repo.getRepoEntry({
			...
			function (err, parentEntry) {
				...
				} else if (![EntryType.RepoRoot, EntryType.Folder].includes(parentEntry.handler)) {
					return cb({
									...
									return Repo.getRepoEntry({
										...
										function (err, currentEntry) {
											if (err || !currentEntry) {
												...
												return Repo.deleteRepoEntry(existingEntry._id, function (err, deletedEntry) {
													if (err) {
														return cb(err);
													} else {
														return Repo.updateRepoEntry(currentEntry._id, {
															parent: parentEntry._id
									...
									return Repo.getRepoEntry({
										...
										function (err, currentEntry) {
											if (err || !currentEntry) {
												...
												return Repo.createRepoEntry(newEntry, overwrite, function (err, createdEntry) {
													if (err) {
														return cb(err);
													} else {
														return FileManager.duplicateFile(createdEntry, currentEntry, function (err, duplicatedFile) {
															if (err) {
																createdEntry.remove(function (err) {
																	if (err) {
...
repoEntrySchema.methods.duplicate = function (newname, overwrite, force, cb) {
	if (![EntryType.VerilogFile, EntryType.TestbenchFile, EntryType.TextFile, EntryType.ImageFile, EntryType.BinaryFile].includes(this.handler)) {
		return cb({
		...
		if (err) {
			...
			return FileManager.getFileEntry({
				...
				if (err) {
					...
					if ((fsEntry.extension != null) && (fsEntry.extension.length > 0)) {
						const suffix = `.${fsEntry.extension}`;
						if (newname.indexOf(suffix, newname.length - suffix.length) !== -1) {
					...
					function (err, existingEntry) {
						if (err) {
							...
							if (!overwrite) {
								...
								return Repo.updateRepoEntry(currentEntry, {
									title: newname
								}, function (err, updatedEntry) {
									if (err) {
										return cb(err);
									} else {
										if (thisHandler === EntryType.Folder) {
											return Repo.deleteRepoEntry(existingEntry._id, function (err, deletedEntry) {
												if (err) {
													return cb(err);
												} else {
													return cb(null, updatedEntry);
												}
											});
										} else {
											return FileManager.renameFile(currentEntry, newname, function (err, updateFileEntry) {
												if (err) {
													return cb(err);
												} else {
													return Repo.deleteRepoEntry(existingEntry._id, function (err, deletedEntry) {
...
repoEntrySchema.methods.getChildren = async function (query = {}, format = 'json', cb) {
	const Repo = require("../controllers/repo");
...
repoEntrySchema.methods.includeInBuild = function (cb) {
	...
	return Repo.updateRepoEntry(this._id, {
		included: true
	}, cb);
};
