...
teamSchema.path('name').validate((name => /^\w+$/gm.test(name)),
	'The team name can only contain letters, numbers and underscores.'
);

teamSchema.path('name').validate((function (name) {
		let needle;
		return (typeof name === 'string') && (name.length) && (needle = name.toLowerCase(), !Array.from(prohibitedNames).includes(needle));
	}),
	'Invalid team name.'
);
		...
		if (err) {
			...
			return Team.getTeamRoleEntry({
				...
				if (err) {
					...
					return Team.updateUserRole(teamId, user._id, level, cb);
				}
			});
		}
	});
