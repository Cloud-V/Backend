	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			const entry = {
				...
				state:
					entryData.state != null ? entryData.state : EntryState.Ready
			};
...
try{
	...
	let recurseEntry = async (entry, targetId) => {
		...
		await entry.p[action](targetId, overwrite, false).then(async processedItem => {
			...
			let validChildren = allChildren.filter(child => child.id !== this.buildDir &&
				![
...
repoSchema.methods.updateEntryAttributes = function(entryId, newAttrs, cb) {
	...
	return this.getEntry(
		...
		function(err, itemEntry) {
			...
			} else if (![EntryType.VCDFile, EntryType.PCFFile].includes(itemEntry.handler)) {
				return cb({
