...
const createProject = function (project, cb) {
	...
	newProject = _.extend(newProject, _.pick(_.omit(project, Utils.nonCloneable), _.pickSchema(projectModel)));
		...
		if (err) {
			...
			return newProject.save(function (err, project) {
				if (err) {
					...
					return assignRole(newProject._id, newProject.owner, AccessLevel.Owner,
						function (err, role) {
							if (err) {
								cb(err);
...
const getProject = function (query, cb) {
	...
	return mongoose.model('Project').findOne(query).sort({
		created: -1
...
const getProjects = function (query, cb) {
	...
	return mongoose.model('Project').find(query).sort({
		created: -1
