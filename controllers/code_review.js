...
const getReview = function (query, cb) {
	...
	return mongoose.model('CodeReview').findOne(query).sort({
		created: 1
...
const getReviews = function (query, cb) {
	...
	return mongoose.model('CodeReview').find(query).sort({
		created: 1
		...
		if (err) {
			...
			return review.getUser(function (err, user) {
				if (err) {
					...
					review.decoded = Buffer.from(review.code, 'base64').toString('utf8');
...
const approveReview = (reviewId, cb) => updateReview(reviewId, {
	state: ReviewState.Approved
}, cb);

const disapproveReview = (reviewId, cb) => updateReview(reviewId, {
	state: ReviewState.Disapproved
}, cb);

const removeReview = (reviewId, cb) => updateReview(reviewId, {
	deleted: true
}, cb);
