...
module.exports.onDisconnect = function (socket) {
    ...
    let index = -1;
    if (
        Array.isArray(global.userSockets[userId]) &&
        (index = global.userSockets[userId].indexOf(socket)) !== -1
    ) {
        global.userSockets[userId].splice(index, 1);
    }
    if (Array.isArray(global.repoSockets[repoId])) {
        for (
            let i = 0, end = global.repoSockets[repoId].length, asc = 0 <= end;
            asc ? i < end : i > end;
            asc ? i++ : i--
        ) {
            index = -1;
    ...
    if (
        Array.isArray(global.userSockets[userId]) &&
        global.userSockets[userId].length === 0
    ) {
        global.userCount--;
...
module.exports.onConnect = async function (socket, user) {
    const { ownerName, repoName } = socket.handshake.query;
    ...
    try {
        ...
        if (
            global.userSockets[userId.toString()] == null ||
            global.userSockets[userId.toString()].length === 0
        ) {
        ...
        if (
            global.repoSockets[repository._id.toString()] == null ||
            global.repoSockets[repository._id.toString()].length === 0
        ) {
