...
const { AccessLevel } = require("../models/repo_access");
const { PrivacyType } = require("../models/repo");
const {
    EntryType,
    EntryAccess,
    EntrySource,
...
const { LicenseType } = require("../models/ip");
...
const createRepo = async (repo, cb) => {
    return new Promise(async (resolve, reject) => {
        repo = _.extend(
            repo,
            _.pick(_.omit(repo, Utils.nonCloneable), _.pickSchema(repoModel))
        );
        ...
        try {
            ...
            const { entry: rootEntry } = await createdRepo.createRoot();
            const { entry: srcFolderEntry } = await createdRepo.createFolder(
                {
            ...
            const { entry: ipFolderEntry } = await createdRepo.createFolder(
                {
                    parent: rootEntry._id,
                    user: createdRepo.owner,
                    title: "IP Cores",
            ...
            const { entry: swHexEntry } = await createdRepo.createFolder(
                {
            ...
            const { entry: docFolderEntry } = await createdRepo.createFolder(
                {
                    parent: rootEntry._id,
                    user: createdRepo.owner,
                    title: "docs",
            ...
            const { entry: readmeFileEntry } = await createdRepo.createTextfile(
                {
            ...
            const { entry: buildFolderEntry } = await createdRepo.createFolder(
                {
                    parent: rootEntry._id,
                    user: createdRepo.owner,
                    title: "build",
...
const cloneRepo = async (sourceRepo, repoData, cb) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const { repoTitle, owner, description } = repoData;
            ...
            sourceFolders.forEach(function (sourceFolder) {
                ...
                if (sourceFolder.parent != null) {
                    ...
                    return childrenMap[sourceFolder.parent].push(
                        sourceFolder._id
                    );
                }
            });
            ...
            sourceEntries.forEach(
                (sourceEntry) => (idMap[sourceEntry._id] = sourceEntry)
            );
            try {
            ...
            const { entry: rootEntry } = await newRepo.createRoot(
                sourceRoot._id
            );
                ...
                (sourceEntry) =>
                    new Promise(async (resolve, reject) => {
                        try {
                            ...
                            const clonedEntry = await createRepoEntry(
                                newEntry,
                                false
                            );
                            let duplicatedFile;
            ...
            const updatedRepo = await updateRepo(newRepo, {
                ...
                swDir:
                    sourceRepo.swDir != null
                        ? cloneMap[sourceRepo.swDir]
                        : null,
                swHexDir:
                    sourceRepo.swHexDir != null
                        ? cloneMap[sourceRepo.swHexDir]
                        : null,
                buildDir: cloneMap[sourceRepo.buildDir],
...
const updateRepoWorkspaceSettings = function (
    repoId,
    themeIndex,
    fontSize,
    cb
...
const createRepoEntry = async (entry, overwrite, cb) => {
    const { model: entryModel } = require("../models/repo_entry");
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (entry.parent == null && entry.handler !== EntryType.RepoRoot) {
                ...
                const entryValidPaths = _.pickSchema(
                    entryModel,
                    Utils.nonCloneable
                );
...
const formatJstree = function (entries, buildDir, swDir, swHexDir) {
    ...
    for (let entry of Array.from(entries)) {
        ...
        if (entry.id.toString() === buildDir.toString()) {
            ...
            swHexDir != null &&
            swHexDir.toString() === entry.id.toString()
        ) {
...
const getRepoDCFStructure = (repoId, cb, format) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            const files = await getRepoStructureByType(
                repoId,
                {
        ...
        try {
            ...
            try {
                ...
                return reject(
                    handleMongooseError(
                        err,
                        "An error occurred while updating the permissions."
                    )
                );
            }
        } catch (err) {
...
const getRepos = async (query, opts = {}, cb) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (!opts.noLimit) {
            dbQuery.limit(ReposPerPage).skip(ReposPerPage * (opts.page || 0));
        }
        try {
            resolve(await dbQuery.exec());
        } catch (err) {
        ...
        try {
            ...
            try {
                ...
                return reject(
                    handleMongooseError(
                        err,
                        "Failed to get shared repositories"
                    )
                );
            }
        } catch (err) {
        ...
        try {
            ...
            try {
                ...
                return reject(
                    handleMongooseError(
                        err,
                        "An error occurred while updating/deleting the repository."
                    )
                );
            }
...
const updateRepoEntry = async (entryId, updates, cb) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const validPaths = _.pickSchema(
                require("../models/repo_entry").model,
                Utils.nonCloneable
            );
            updates = _.pick(updates, validPaths);
        ...
        try {
            ...
            const promises = _.map(
                ...
                (child) =>
                    new Promise(async (resolve, reject) => {
                        try {
                            const deletedEntry = await deleteRepoEntry(
                                child._id
                            );
                            const deletedFileEntry =
                                await FileManager.clearFileEntry(deletedEntry);
                            return resolve();
                        } catch (err) {
                            return reject(err);
                        }
            ...
            const deletedFileEntry = await FileManager.clearFileEntry(
                deletedEntry
            );
...
const updateEntryAttributes = (entryId, content, cb) =>
    updateRepoEntry(
        entryId,
        {
            attributes: content,
...
const updateEntryData = (entryId, content, cb) =>
    updateRepoEntry(
        entryId,
        {
            data: content,
...
const getRepoContributors = async (repo, cb) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const promises = _.map(
                roles,
                ({ _id, accessLevel, createdAt, updatedAt, user: userId }) =>
                    new Promise(async (resolve, reject) => {
                        try {
                            const user = (
                                await User.getUser({
...
const getTopRepos = function (limit, cb) {
    ...
    return mongoose
        ...
        .limit(limit)
        .exec(function (err, featuredRepos) {
            if (err) {
                console.error(err);
                return cb({
                ...
                return mongoose
                    ...
                    .limit(newLimit)
                    .exec(function (err, unfeaturedRepos) {
                        if (err) {
                            console.error(err);
                            return cb({
                                error: "An error occurred while retrieving repositories.",
                            ...
                            return cb(
                                null,
                                featuredRepos.concat(unfeaturedRepos)
                            );
                        }
                    });
            }
...
const countRepoEntries = function (query, cb) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            return resolve(
                await mongoose.model("RepoEntry").countDocuments(query)
            );
        } catch (err) {
            return reject(
                handleMongooseError(
    ...
    return repo.getIP(function (err, existingIp) {
        ...
        if (err) {
            ...
            if (typeof ip.inputs === "string") {
                newIp.inputs = ip.inputs.split(/\s*,\s*/gim);
            ...
            if (typeof ip.outputs === "string") {
                newIp.outputs = ip.outputs.split(/\s*,\s*/gim);
            ...
            if (
                ip.inputs != null &&
                (typeof ip.inputs === "string" || Array.isArray(ip.inputs))
            ) {
                updated = true;
                if (typeof ip.inputs === "string") {
                    existingIp.inputs = ip.inputs.split(/\s*,\s*/gim);
                } else if (Array.isArray(ip.inputs)) {
                    existingIp.inputs = [];
                    for (input of Array.from(ip.inputs)) {
                        if (typeof input !== "string") {
            ...
            if (
                ip.outputs != null &&
                (typeof ip.outputs === "string" || Array.isArray(ip.outputs))
            ) {
                updated = true;
                if (typeof ip.outputs === "string") {
                    existingIp.outputs = ip.outputs.split(/\s*,\s*/gim);
                } else if (Array.isArray(ip.outputs)) {
                    existingIp.outputs = [];
                    for (output of Array.from(ip.outputs)) {
                        if (typeof output !== "string") {
                    ...
                    return User.getUser(
                        ...
                        function (err, user) {
                            if (err) {
                                ...
                                console.error(
                                    `Cannot find the owner of IP ${ip.title}`
                                );
                                return callback();
                            } else {
                                const validPaths = _.pickSchema(ipModel, [
                                    "repo",
                                    "user",
                                    "created",
                                    "__v",
        ...
        try {
            ...
            try {
                ...
                return reject(
                    handleMongooseError(
                        err,
                        "An error occurred while creating the version."
                    )
                );
            }
            ...
            let newVersion = new versionModel(
                _.pick(
                    _.omit(versionData, Utils.nonCloneable),
                    _.pickSchema(versionModel)
                )
            );
                    ...
                    if (err) {
                        ...
                        return User.getUser(versionUser, function (err, user) {
                            if (err) {
                                ...
                                if (
                                    typeof newVersion.title !== "string" ||
                                    newVersion.title.trim() === ""
                                ) {
                                    return cb({
                                ...
                                if (
                                    typeof newVersion.number !== "string" ||
                                    newVersion.number.trim() === ""
                                ) {
                                    return cb({
                                ...
                                return repo.clone(
                                    ...
                                    function (err, clonedRepo) {
                                        if (err) {
                                            ...
                                            newVersion.versionRepo =
                                                clonedRepo._id;
                                            return newVersion.save(function (
                                                err,
                                                savedVersion
                                            ) {
                                                if (err) {
...
const getRepoVersions = async (query, opts = {}, cb) => {
    ...
    return new Promise(async (resolve, reject) => {
        const dbQuery = versionModel.find(query).sort(
            opts.sort || {
                createdAt: -1,
            }
        );
