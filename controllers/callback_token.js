...
module.exports.createToken = (createToken = function (tokenData, cb) {
	...
	newToken = _.extend(newToken, _.pick(_.omit(tokenData, Utils.nonCloneable), _.pickSchema(callbackTokenModel)));
...
module.exports.getToken = (getToken = function (query, cb) {
	...
	return callbackTokenModel.findOne(query).sort({
		created: -1
	...
	getToken({
		...
		function (err, token) {
			if (err) {
				...
				const validPaths = _.pickSchema(callbackTokenModel, Utils.nonCloneable);
