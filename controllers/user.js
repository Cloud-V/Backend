    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!passwordValidator().test(user.password)) {
                return reject({
                    error: "New password should have at least 8 characters and contain at least one letter and one numeral.",
                });
            }
            try {
...
const countUsers = function (query, cb) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            return resolve(
                await mongoose.model("User").countDocuments(query).exec()
            );
        } catch (err) {
            return reject(
                handleMongooseError(
...
const generateDefaultUserProfile = async (user, cb) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const avatarFile = await FileManager.createMediaFile(
                { buffer: generatedAvatar },
                {
                    user: user._id,
