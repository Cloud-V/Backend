...
let externalUser = new User('external_user', 'loudPatch66', 'external_user@cloudv.io');
let versionUser = new User('version_user', 'loudPatch66', 'version_user@cloudv.io');

// Sessions
let sessionKey = process.env.CLOUDV_SESSION_KEY || "Y6m5NtJVMo0yO0wN4v1gO164a9266TZf";

let jwtSecret = process.env.CLOUDV_JWT_SECRET || "284ES2g3nmt7B668v57m45AZ8292Wf";
...
let docker = {
	...
	workspacePath: process.env.CLOUDV_WORKSPACE_DOCKER_PATH || "/tmp/ws",
	stdcellsPath:
		process.env.CLOUDV_WORKSPACE_DOCKER_STDCELLS_PATH ||
		"/tmp/stdcells",
	stdcellsModelsPath:
		process.env.CLOUDV_WORKSPACE_DOCKER_STDCELLS_MODELS_PATH ||
		"/tmp/stdcells-models",
	buildPath:
		process.env.CLOUDV_WORKSPACE_DOCKER_BUILD_PATH || "/tmp/build",
...
let consolidatedProcessing = JSON.parse(process.env.CLOUDV_CONSOLIDATE_PROCESSING || "false");

let defaultPort =
	servedComponent === "api" ? 3000:
	servedComponent === "processing" ? 4040:
	9999
	;
