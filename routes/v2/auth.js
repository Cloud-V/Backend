...
router.post("/google/callback", async (req, res, next) => {
    ...
    // });
    try {
        const { id, displayName, avatar, email, username } =
        ...
        if (existing) {
            const { user, token } = await User.getLoginUser({
                _id: existing._id,
            });
            user.token = token;
            ...
            const userData = {
                ...
                username: `${username}_${id}_${shortid.generate()}`.replace(
                    /[^\w]+/m,
                    "_"
                ),
            ...
            const { user, token } = await User.createUser(userData);
    ...
    try {
        ...
        if (existing) {
            ...
            const userData = {
                email,
                password: `1234Ab${shortid.generate()}`,
                username: `${login}_${id}_${shortid.generate()}`.replace(
                    /[^\w]+/m,
                    "_"
                ),
...
router.post("/reset", async (req, res, next) => {
    ...
    try {
        ...
        const { user, token } = await User.resetPassword(
            targetUser,
            resetToken,
            password
        );
...
router.post("/forgot", async (req, res, next) => {
    const { username, captcha_token } = req.body;
    ...
    try {
        ...
        const { user, resetToken } = await User.forgotPassword(targetUser);
        const profile = await User.getUserProfile({
            user: user._id,
        });
        const merged = _.extend(
