...
router.post("/compile", restrict, async function (req, res, next) {
    try {
        ...
        let overwrite = req.body.overwrite != null ? req.body.overwrite : false;
        ...
        if (process.env.CLOUDV_DISABLE_DOCKER !== "1") {
            ...
            if (action === "synthesize") {
                const synthesisUrl = proc.url + proc.synthesizePath;
                    ...
                    return repo.createNetlist(
                        ...
                        function (err, netlistResult) {
                            if (err) {
                                ...
                                const { entry: netlistEntry } = netlistResult;
                                return repo.createSynthesisReport(
                                    req.user,
                                    name,
                                    "",
                                    ...
                                    function (err, reportResult) {
                                        if (err) {
                                            console.error(err);
                                            netlistEntry.remove(function (err) {
                                                if (err) {
                ...
                const validateUrl = proc.url + proc.validatePath;
                requestBody = _.clone(req.body);
                delete requestBody.user;
                requestBody.username = ownerName;
                requestBody.reponame = repoName;
                ...
                return repo.getEntry(
                    ...
                    function (err, pcfEntry) {
                        if (err) {
                            ...
                            const compileUrl = proc.url + proc.bitstreamPath;
                            requestBody = _.clone(req.body);
                            delete requestBody.user;
                            requestBody.username = ownerName;
                            requestBody.reponame = repoName;
                ...
                try {
                    ...
                    res.socket.setTimeout(requestTimeout);
                    let { httpResponse, body } = await new Promise(
                        (resolve, reject) =>
                            request.post(
                                {
                                    url: compileUrl,
                    ...
                    if (!listContent) {
                        return res.status(200).json({
                            parentId: hexEntry.parent,
                            fileId: hexEntry._id,
                            fileName: hexEntry.title,
                            fileType: hexEntry.handlerName,
                            content: hexContent,
                            list: null,
                            log: compilationLog,
                        });
                    ...
                    if (listResult) {
                        ...
                        list = {
                            parentId: listEntry.parent,
                            fileId: listEntry._id,
                            fileName: listEntry.title,
                            fileType: hexEntry.handlerName,
                            content: listContent,
                        };
                    }
                ...
                function (err, tempPath, files, reverseMap) {
                    if (err) {
                        ...
                        if (action === "synthesize") {
                            ...
                            return res.status(500).json({
                                error: "Single file validation is no longer supported.",
                            });
                            for (key in req.body) {
                                value = req.body[key];
                                if (typeof value === "object") {
                    ...
                    return repo.getEntryByPath(
                        ...
                        function (err, result) {
                            if (err) {
                                ...
                                return res.status(400).json({
                                    error: "Current file type is not supported for reading.",
                                });
                            } else {
...
router.get("/boards", restrict, function (req, res, next) {
    ...
    return Repo.accessRepo(ownerName, repoName, userId, next)
        .then((result) => {
        ...
        .then((readable) => {
            if (!readable) {
                return res
                    .status(403)
                    .json({ error: "No read access to this repository." });
            }
...
router.get("/stdcell", restrict, async function (req, res, next) {
    ...
    try {
        let accessInfo = await Repo.accessRepo(
            ownerName,
            repoName,
            userId,
            next
        ...
        for (let scl of scls) {
            ...
            if (fs.existsSync(constraintsPath)) {
                let constraintsStr = fs.readFileSync(constraintsPath, {
                    encoding: "utf8",
                });
                        ...
                        return fs.readdir(
                            ...
                            function (err, strategiesFiles) {
                                if (err) {
                                    console.error(err);
                                    return res.status(500).json({
                                        error: "An error has occurred while attempting to retrieve the synthesis strategies.",
                                    });
                                } else {
                    ...
                    if (!readable) {
                        ...
                        return Repo.getAvailableIPs(function (err, ips) {
                            if (err) {
                                console.error(err);
                                return res.status(500).json({
                                    error: "An error has occurred while attempting to retrieve available IP Cores.",
                                });
                            } else {
                                return res.status(200).json({
                                    ips,
                ...
                if (err) {
                    ...
                    if (isDnD) {
                        if (process.env.DISABLE_EXTERNAL_DND === "1") {
                            ...
                            return res.status(500).json({
                                error: "Drag and drop file upload is currently disabled.",
                            });
                        }
                    }
...
router.post("/ajax", restrict, async function (req, res, next) {
    try {
        ...
        if (action === "create") {
            ...
            if (req.body.parent == null || req.body.parent.trim() === "") {
                return res.status(400).json({
                    error: "Missing parameter 'parent'",
                });
            }
            ...
            if (type === "verilog") {
                ...
                return repo.createVerilogModule(
                    ...
                    function (err, result) {
                        if (err) {
                            ...
                            const { entry: newEntry, content: moduleContent } =
                                result;
                ...
                if (process.env.CLOUDV_DISABLE_DOCKER === "1") {
                    return res.status(500).json({
                        error: "Testbench generation is currently disabled.",
                    });
                }
                ...
                const isBlank = req.body.blank === true ? true : false;
                if (isBlank) {
                    ...
                    validateUrl = proc.url + proc.validateTopModulePath;
                    requestBody = _.clone(req.body);
                    delete requestBody.user;
                    requestBody.username = ownerName;
                    requestBody.reponame = repoName;
                        ...
                        function (err, httpResponse, body) {
                            if (err) {
                                ...
                                if (!logs || !logs.errors) {
                                    ...
                                    console.error(body);
                                    return res.status(500).json({
                                        error: "There are errors in the source file, please validate its contents first.",
                                        errors: [
                ...
                return repo.createTextfile(
                    ...
                    function (err, result) {
                        if (err) {
                            ...
                            const { entry: newEntry, content: fileContent } =
                                result;
                            return res.status(201).json({
                                fileId: newEntry._id,
                                parentId: newEntry.parent,
                ...
                if (
                    ...
                    return res.status(500).json({
                        error: 'Missing or invalid parameter "target"',
                    });
                }
                return repo.createLinkerFile(
                    parentId,
                ...
                const generatedContent =
                    busReader.generateDefaultSoC(busTemplate);
                ...
                return repo.createDCF(
                    ...
                    function (err, result) {
                        if (err) {
                            ...
                            const { entry: newEntry, content: dcfContent } =
                                result;
                            return res.status(201).json({
                                fileId: newEntry._id,
                                parentId: newEntry.parent,
                ...
                if (!targetBoard) {
                    return res.status(500).json({
                        error: "Cannot find the specified board model.",
                    });
                }
                ...
                const { entry: newEntry, content: pcfContent } = result;
            ...
            if (req.body.target == null || req.body.target.trim() === "") {
                return res.status(400).json({
                    error: "Missing parameter 'target'",
                });
            }
            if (req.body.item == null || !Array.isArray(req.body.item)) {
                return res.status(400).json({
            ...
            ({ item } = req.body);
            return repo.deleteMultiple(
                item,
                false,
                function (err, deletedItems) {
            ...
            if (req.body.item == null || req.body.item.trim() === "") {
                return res.status(400).json({
                    error: "Missing parameter 'item'",
                });
            }
            if (req.body.newname == null || req.body.newname.trim() === "") {
                return res.status(400).json({
            ...
            return repo.duplicateEntry(
                ...
                function (err, duplicatedEntry) {
                    if (err) {
                        ...
                        const fileType = duplicatedEntry.handlerName;
                        return res.status(200).json({
                            ...
                            function (err, exists) {
                                if (err) {
                                    ...
                                    const constrContent = {
                                        ...
                                        inputDrivingCell:
                                            validConstr.inputDrivingCells[0],
                                        maxTransition:
                                    ...
                                    for (let opt of Array.from(numberOpts)) {
                                        if (content[opt] != null) {
                                            ...
                                            if (isNaN(optValue)) {
                                                return cb({
                                                    error: `Invalid constraint value ${content[opt]} for option ${opt}.`,
                                                });
                                            }
                                            if (
                                                optValue <
                                    ...
                                    if (
                                        ...
                                        ).includes(content.inputDrivingCell)
                                    ) {
                                        constrContent.inputDrivingCell =
                                            content.inputDrivingCell;
                                    }
                                    content = JSON.stringify(constrContent);
                        ...
                        return repo.updateEntryAttributes(
                            ...
                            function (err, updatedEntry) {
                                if (err) {
                                    ...
                                    const fileType = updatedEntry.handlerName;
                                    return res.status(200).json({
                                        fileId: updatedEntry._id,
                                        fileName: updatedEntry.title,
                                        parentId: updatedEntry.parent,
                                        fileType,
            ...
            if (req.body.module == null || req.body.module.trim() === "") {
                return res.status(400).json({
                    error: "Missing parameter 'module'",
                });
            }
            ...
            return repo.setTopModule(item, module, function (err, topModule) {
                if (err) {
                    console.error(err);
                    return res.status(500).json(err);
                } else {
            ...
            if (sourceEntry.handlerName === "vcd") {
                try {
                    const vcdData = JSON.parse(sourceEntry.data);
                    req.body.item = item = vcdData.source;
                    req.body.netlist = isNetlist = vcdData.netlist || false;
                    req.body.netlistId = netlistId = vcdData.netlistId || "";
                    req.body.stdcell = stdcell = vcdData.stdcell || "";
                    // req.body.level = (level = vcdData.level || 0);
                } catch (error) {
                    const e = error;
                    console.error(err);
                    return res.status(500).json({
                        error: "Cannot locate source testbench.",
                    });
                }
            }
            ...
            ({ stdcell } = req.body);
            overwrite = req.body.overwrite != null ? req.body.overwrite : false;
            ...
            if (
                ...
                return res.status(500).json({
                    error: "The timing constraints must have the attribute clock.",
                });
            }
                                        ...
                                        function (err, stdCellData) {
                                            if (err) {
                                                ...
                                                return STAParser.parseLiberty(
                                                    stdCellData,
                                                    function (err, stdcells) {
                                                        if (err) {
                                                            return res
                                                                .status(500)
                                                                .json({
                                                                    error: "Failed to parse the liberty file.",
                                                                });
                                                        } else {
                                                                ...
                                                                function (
                                                                    ...
                                                                    if (err) {
                                                                        return res
                                                                            ...
                                                                            .json(
                                                                                {
                                                                                    error: "Failed to parse the netlist file.",
                                                                                }
                                                                            );
                                                                    } else {
            ...
            parentId = req.body.parent;
            if (req.body.type == null || req.body.type.trim() === "") {
                return res.status(400).json({
                    error: "Missing parameter 'type'",
                });
            }
            ...
            let method =
                action == "include" ? "includeInBuild" : "excludeFromBuild";
            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (let item of items) {
                    ...
                    if (!entry.includable) {
                        return reject(
                            `Operation ${action} is not applicable to ${item}.`
                        );
                    }
    ...
    return Repo.accessRepo(
        ...
        function (err, result) {
            if (err) {
                ...
                if (
                    req.body.outputs == null ||
                    req.body.outputs.trim() === ""
                ) {
                    return res.status(400).json({
                ...
                return Repo.publishRepoIP(
                    repo,
                    newIp,
                    function (err, createdIp) {
                        if (err) {
                            console.error(err);
...
router.get("/versions/download/:versionId", async (req, res, next) => {
    ...
    try {
        ...
        const downloadName = `${version.repoTitle}-${version.title}-${version.number}.zip`;
