  ...
  try {
    ...
    if (ex.name === 'MissingSchemaError') {
      ...
      counterSchema = new mongoose.Schema({
        ...
        baseIdField: {type: String, require: true},
        baseIdValue: {type: mongoose.Schema.ObjectId, require: true}
      });

      // Create a unique index using the "field" and "model" fields.
      counterSchema.index({ field: 1, model: 1 }, {required: true, index: -1 });
...
exports.plugin = function (schema, options) {
  ...
  var nextCount = function (baseIdValue, callback) {
    IdentityCounter.findOne({
      model: settings.model,
      field: settings.field,
      baseIdField: settings.baseIdField,
      baseIdValue: baseIdValue
    }, function (err, counter) {
  ...
  var resetCount = function (baseIdValue, callback) {
    IdentityCounter.findOneAndUpdate(
      { model: settings.model, field: settings.field, baseIdField: settings.baseIdField, baseIdValue: baseIdValue},
  ...
  schema.pre('save', function (next) {
    var doc = this;
    var baseIdValue = doc[settings.baseIdField];

    if (doc.isNew) {
     IdentityCounter.findOne(
        { model: settings.model, field: settings.field, baseIdField: settings.baseIdField, baseIdValue: doc[settings.baseIdField]},
        function (err, counter) {
          if (!counter) {
            // If no counter exists then create one and save it.
            counter = new IdentityCounter({ model: settings.model, field: settings.field, count: settings.startAt - settings.incrementBy, baseIdField: settings.baseIdField, baseIdValue: doc[settings.baseIdField]});
                  ...
                  if (ready) {
                    ...
                    if (typeof doc[settings.field] === 'number') {
                      IdentityCounter.findOneAndUpdate(
                        ...
                        { model: settings.model, field: settings.field, baseIdField: settings.baseIdField, baseIdValue: doc[settings.baseIdField], count: { $lt: doc[settings.field] } },
