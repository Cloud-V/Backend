...
const main = async () => {
    let fileName = "",
        content = "";
    let createdSuccessfully = 0,
        fsidsNotInGFSFiles = 0,
        fsidsNotInGFSChunks = 0,
        errorReadingFileContent = 0;
    ...
    try {
        ...
        for (let cur of res) {
            ...
            if (!file || !file.filename) {
                console.log(
                    `${fileName} repo file fsid does not exist in gridfs-stream files`
                );
                fsidsNotInGFSFiles++;
                continue;
            }
            ...
            gfs.exist({ _id: cur.fsId }, function (err, found) {
                ...
                if (!found) {
                    console.log(
                        `${fileName} repo file fsid does not exist in gridfs-stream chunks`
                    );
                    fsidsNotInGFSChunks++;
...
main()
    .then((res) => {
        ...
        console.log(
            `\t- Fsids Not In GFS Files collection: ${res.fsidsNotInGFSFiles}`
        );
        console.log(
            `\t- Fsids Not In GFS Chunks collection: ${res.fsidsNotInGFSChunks}`
        );
        console.log(
            `\t- Error Reading File Content: ${res.errorReadingFileContent}`
        );
    })
    .catch();
